...
const getServerTime = (url, opts) => {
    const options = {
        ...
        ...Object.keys(opts || {}).reduce(
            (finalOptions, key) => (typeof opts[key] !== 'undefined' && opts[key] !== null
                ? {
                    ...finalOptions,
                    [key]: opts[key],
                }
    ...
    return fetch(clockUrl, {
        ...
        .then((response) => {
            ...
            const { serverTimestamp, serverClientRequestDiffTime } = options.parseResponse(
                response,
            );
            if (
            ...
            const responseTime = (serverClientRequestDiffTime
                    - nowTimeStamp
